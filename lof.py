...
def k_reach_distance(k, instance1, instance2, instances, distance_function=distance_euclidean):
...
def local_reach_density(min_pts, instance, instances, **kwargs):
    ...
    for i, neighbour in enumerate(neighbours):
        reachability_distances_array[i] = k_reach_distance(min_pts, instance, neighbour, instances, **kwargs)
...
def local_outlier_factor(min_pts, instance, instances, **kwargs):
    ...
    Signature: (int, (attr1, attr2, ...), ((attr_1_1, ...),(attr_2_1, ...), ...)) -> float"""
    (k_distance_value, neighbours) = k_distance(min_pts, instance, instances, **kwargs)
    instance_lrd = local_reach_density(min_pts, instance, instances, **kwargs)
    lrd_ratios_array = [0] * len(neighbours)
    for i, neighbour in enumerate(neighbours):
        instances_without_instance = set(instances)
        instances_without_instance.discard(neighbour)
        neighbour_lrd = local_reach_density(min_pts, neighbour, instances_without_instance, **kwargs)
...
def outliers(k, instances, **kwargs):
    ...
    for i, instance in enumerate(instances_value_backup):
        ...
        _lof = LOF(instances, normalize=False, **kwargs)
        # _lof = LOF(instances, normalize=True, **kwargs)
        lof_value = _lof.local_outlier_factor(k, instance)
        if lof_value > 1:
            _outliers.append({"lof": lof_value, "instance": instance, "index": i})
    _outliers.sort(key=lambda o: o["lof"], reverse=True)
    return _outliers
